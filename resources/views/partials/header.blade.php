<div class="container nav">
  <a class="brand" href="{{ url('/') }}">
    <img class="brand-logo" src="{{ asset($siteSettings['site_logo'] ?? 'assets/img/logo.png') }}" alt="{{ $siteSettings['site_name'] ?? 'Legacy Leather Works' }}">
    ...
    @auth
      <div style="position:relative;display:flex;gap:8px;align-items:center;">
        @if(auth()->user()->is_admin)
          <a class="cartBtnTop" href="{{ route('admin.dashboard') }}" style="display:inline-flex;align-items:center;gap:6px;background:rgba(107,63,42,.15);border-color:rgba(107,63,42,.3);">
            <i class="bi bi-shield-check"></i>
            <span>Admin</span>
          </a>
        @else
          <a class="cartBtnTop" href="{{ route('dashboard') }}" style="display:inline-flex;align-items:center;gap:6px;">
